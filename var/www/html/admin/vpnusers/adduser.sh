...
# 	sed -i -e '$a\'  /etc/openvpn/server/server.conf
# 	sed -i -e '$a\#subnet for '"$CLIENTNAME"':'  /etc/openvpn/server/server.conf
# 	sed -i -e '$a\route '"$SUBNET"' '"$NETMASK"'' /etc/openvpn/server/server.conf
...
# 	echo "iroute $SUBNET $NETMASK" > /etc/openvpn/client/$CLIENTNAME
    ...
    if [[ -f pki/issued/$CLIENTNAME.crt ]]; then
        echo "Error: certificate $CLIENTNAME alread exists!"
        exit 1
    fi
    ...
    echo "<ca>" >> $OUTDIR/$CLIENTNAME.ovpn
    cat pki/issued/ca.crt >> $OUTDIR/$CLIENTNAME.ovpn
    echo "</ca>" >> $OUTDIR/$CLIENTNAME.ovpn

    echo "<cert>" >> $OUTDIR/$CLIENTNAME.ovpn
    cat pki/issued/$CLIENTNAME.crt >> $OUTDIR/$CLIENTNAME.ovpn
    echo "</cert>" >> $OUTDIR/$CLIENTNAME.ovpn

    echo "<key>" >> $OUTDIR/$CLIENTNAME.ovpn
    cat pki/private/$CLIENTNAME.key >> $OUTDIR/$CLIENTNAME.ovpn
    echo "</key>" >> $OUTDIR/$CLIENTNAME.ovpn
